...
new function() {
    ...
    Objs = function(classpath, second, third) {
        ...
        // Retrieve
        //
        if (typeof classpath != Tstring) throw Error($InvalidClasspath + classpath);
        ...
        if (i) {
             ...
             */
            if (!superclass) throw Error($NonExistentSuperClass + second);
            /*
             * The developer try to inherit from a registered superclass
             * but with an undefined protobject.
             */
            if (!(protobject = third)) throw Error($InvalidProtobject + classpath);
        }
        ...
        func = map[path] = function() {
            ...
            if (!func[$extending]) {
                 ...
                 */
                if (!func[$constructing] && func.prototype.initialize) func.prototype.initialize.apply(this, arguments);
            }
        };
        ...
        for (propName in protobject)
            if (protobject.hasOwnProperty(propName)) temp[propName] = protobject[propName];
